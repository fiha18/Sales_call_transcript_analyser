...
def merge_chunk_summaries(chunk_response_list,summary_format="paragraph",word_limit = 1000,language="urdu"):
    """
    ...
    """
    system_message = summarizer_prompt.get_merge_response_system_message(chunk_response_list,summary_format,word_limit,language)
...
def generate_transcript_summary_list(file_path,summary_format="paragraph",language="urdu"):
    """
    ...
    for chunk in enumerate(chunks, start=1):
        system_message = summarizer_prompt.get_summarizer_system_message(language)
        prompt_message = summarizer_prompt.get_summarizer_user_prompt(chunk,utils.get_major_context,summary_list,language)
...
def perform_call_transcript_summary_generation():
    ...
    language = command_line_args[4] if len(command_line_args) > 4 else "english"
    ...
    summary_list = generate_transcript_summary_list(transcript_file_path,summary_format,language)
    # folder path to save summary 
    folder_path="generated_summaries"
    # Create the folder if it doesn't exist
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    file_name = input_file_name.replace(".txt",f"_{summary_format}_in_{language}_summary_list.txt")
    file_path = os.path.join(folder_path, file_name)
    if os.path.exists(file_name):
        print(f"Output file {os.path.basename(file_path)} already exists, skipping.")
        return
    # Creating final response 
    summary = merge_chunk_summaries(summary_list,summary_format,word_limit,language)  
